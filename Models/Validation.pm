...
sub checkRegForm
{
    my($self, %hash) = @_;
     ...
     return "uncorrect login" if !($hash{'login'} = $self->clearWord($hash{'login'})); 
     return "uncorrect email" if !($hash{'email'} = $self->clearEmail($hash{'email'}));
     return "uncorrect password" if !($hash{'password'} = $self->clearWord($hash{'password'}));
     $hash{'password_v'} = $self->clearWord($hash{'password_v'});
     return "password mismatch" if !($hash{'password'} == $hash{'password_v'});
